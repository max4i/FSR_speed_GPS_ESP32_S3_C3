...
<title>FSR_ACC - Informacje</title>
<style>
...
<table>
  ...
  <tr><td>ADXL345 SCL</th><td>8</th><td>I2C zegar</th></tr>
  <tr><td>ADXL345 SDA</th><td>9</th><td>I2C dane</th></tr>
...
<div class="formula">
H(s) = 1 / (s² + 1.414s + 1)<br>
Częstotliwość odcięcia: fc = 20Hz
</div>
...
<div class="formula">
|xi - median| > 3 × MAD → outlier<br>
gdzie: MAD = median(|xi - median|)
</div>
...
<div class="formula">
Współczynniki: [-2, 3, 6, 7, 6, 3, -2] / 21
</div>
...
<li>Przy małym przyspieszeniu i dobrym HDOP → <strong>90% GPS, 10% ACC</strong> (wygładzenie)</li>
<li>Przy dużym przyspieszeniu (zmiana prędkości) → <strong>30% GPS, 70% ACC</strong> (szybka reakcja)</li>
<li>Przy słabym sygnale GPS (HDOP > 1.4) → <strong>20% GPS, 80% ACC</strong></li>
<li>Przy braku sygnału GPS → <strong>100% ACC</strong> (krótkotrwale, do 5s)</li>
</ul>
<div class="formula">
v_fused = α × v_gps + (1-α) × (v_acc + ∫a×dt)<br>
α = dynamic (0.2 do 0.9 w zależności od warunków)
</div>
...
<div class="formula">
Predykcja: x̂ₖ = F·x̂ₖ₋₁ + B·uₖ<br>
Korekcja: x̂ₖ = x̂ₖ + K·(zₖ - H·x̂ₖ)<br>
Wzmocnienie Kalmana: K = Pₖ·Hᵀ·(H·Pₖ·Hᵀ + R)⁻¹
</div>
...
<div class="formula">
K_sat = 1 - (SAT - 4) / 30<br>
Ograniczenie: 0,3 ≤ K_sat ≤ 1,0
</div>
<p>Dla SAT=26: K_sat = 1 - 22/30 = 0,27 → ograniczone do 0,3</p>
<p>Dla SAT=10: K_sat = 1 - 6/30 = 0,80</p>
...
<div class="formula">
K_hdop = HDOP / 1,0<br>
Ograniczenie: 0,5 ≤ K_hdop ≤ 2,0
</div>
<p>Dla HDOP=0,5: K_hdop = 0,5</p>
<p>Dla HDOP=1,4: K_hdop = 1,4</p>
...
<div class="formula">
K_filtr = 1 / √n = 1 / √3 = 0,577
</div>

<h3>Wzór końcowy na dokładność statyczną</h3>
<div class="formula">
Dokładność [km/h] = 0,36 × K_sat × K_hdop × K_filtr
</div>
...
<div class="formula">
Dokładność_dyn [km/h] = 0,36 × K_sat × K_hdop × K_filtr × 2,5
</div>
<p>Współczynnik 2,5 wynika z opóźnienia filtra i dynamiki akcelerometru.</p>

<h3>Przykład dla SAT=20, HDOP=0,7:</h3>
<p>Dokładność statyczna = 0,068 km/h<br>
Dokładność dynamiczna = 0,068 × 2,5 = <strong>0,170 km/h</strong></p>
...
<ul style="margin-left:20px;">
<li><span class="badge">0,03 - 0,05</span> Doskonała</li>
<li><span class="badge">0,05 - 0,10</span> Bardzo dobra</li>
<li><span class="badge">0,10 - 0,20</span> Dobra</li>
<li><span class="badge">0,20 - 0,50</span> Przeciętna</li>
<li><span class="badge">0,50 - 1,00</span> Słaba (sprawdź antenę GPS)</li>
</ul>
</div>
...
<table>
  ...
  <tr><td>Temperatura 0°C</th><td>Włącz czujnik w ustawieniach, sprawdź połączenia</th></tr>
  <tr><td>VMAX nie zapisuje</th><td>HDOP > 1,4 lub SAT < 6 – popraw sygnał GPS</th></tr>
